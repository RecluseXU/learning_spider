...
def extrace_object_demo():#视频读入
    capture = cv.VideoCapture('video/HoldARedThing.mp4')
...
def three_type(image):
    ...
    cv.imwrite('result/4-2blue.png',b)
    cv.imshow('green',g)#显示绿色通道
    cv.imwrite('result/4-2green.png',g)
    cv.imshow('red',r)#显示红色通道
    cv.imwrite('result/4-2red.png',r)
    
    image[:,:,0]=0#把一个通道弄没
    cv.imshow('split change',image)
    cv.imwrite('result/4-2split_change.png',image)
    
    image = cv.merge([b,g,r])#将三个通道合在一起，就会变成原本的图片
    cv.imshow('mix again',image)
    cv.imwrite('result/4-2mix_again.png',image)


src = cv.imread('picture/zombe540x540.jpg')
