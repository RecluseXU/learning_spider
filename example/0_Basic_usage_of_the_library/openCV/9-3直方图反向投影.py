...
def back_projection_demo(sample, target):  # 直方图反向投影
    ...
    sample_Hist = cv.calcHist([sample_hsv], [0, 1], None, [
                              12, 36], [0, 180, 0, 256])
    ...
    dst = cv.calcBackProject([target_hsv], [0, 1],
                             sample_Hist, [0, 180, 0, 256], 1)
    cv.imshow('backprojection_demo', dst)
    cv.imwrite(
        'example/0_Basic_usage_of_the_library/openCV/result/9-3backprojection.png', dst)


src1 = cv.imread(
    'example/0_Basic_usage_of_the_library/openCV/picture/angle_cloths_material.png')
src = cv.imread(
