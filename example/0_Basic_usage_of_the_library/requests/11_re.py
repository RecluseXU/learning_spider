...
# "."默认不匹配换行，加上re.S就匹配换行了
# re.S 忽略大小写
three = "321m\n123a1m23A"
pattern = re.compile("m(.*)a", re.S)
...
# 范围
five = "1234567890"
pattern = re.compile('[139]')
result = pattern.match(five)
print("five\t", result)

'''
匹配方法：
    match   从头开始匹配，匹配一次
    findall 查找符合要求的字符串，以得到内容
    sub     替换字符串
    split   拆分
...
原文链接：https://blog.csdn.net/ZYC88888/article/details/98479629
'''