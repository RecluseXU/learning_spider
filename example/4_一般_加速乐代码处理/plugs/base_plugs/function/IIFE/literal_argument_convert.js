...
const visitor = {
    "CallExpression"(path){
        ...
        if(!types.isFunctionExpression(exp_node.callee))return;
        for(arg_ind in exp_node.arguments){

            if(!types.isLiteral(exp_node.arguments[arg_ind]))continue;
            let {type, value} = exp_node.arguments.splice(arg_ind)[0];
            let neo_var_name = exp_node.callee.params.splice(arg_ind)[0].name;
            let neo_var_node = types.variableDeclaration(
                "var",
                [
                    types.VariableDeclarator(
                        types.Identifier(neo_var_name),
                        types[type](value),
                    )
                ]
            );
            path.get('callee.body.body.0').insertBefore(neo_var_node)
        }
    },
}

const plug = new BasePlug(
    'literal argument convert',
    visitor,
    '自调用函数传入 Literal 实参时，Literal 实参转为函数内部定义，去除形参'
);
